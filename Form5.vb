...
Public Class Form5
    ...
    Private Sub Deletebtn2_Click(sender As Object, e As EventArgs) Handles Deletebtn2.Click
        If LV3.SelectedItems.Count > 0 Then
            For Each item As ListViewItem In LV3.SelectedItems

                ' Delete the record from the database
                Dim connectionString As String = "server=localhost; user id=root; password=; database=productsdb;"
                Dim productCode As String = item.SubItems(0).Text 'Product Code Column
                Dim query As String = "DELETE FROM productsales WHERE Product_Code = @Product_Code"
    ...
    Private selectedItems As New List(Of ListViewItem)

    Private Sub LV3_ItemCheck(sender As Object, e As ItemCheckEventArgs) Handles LV3.ItemCheck
        Dim item As ListViewItem = LV3.Items(e.Index)
